...
This tool downloads media from visible posts on Twitter (now X) and saves it into a local Obsidian Vault. It combines the **Twitter Media Collector** Chrome extension and the **Twitter Media Archive Companion** Obsidian plugin.
...
1. Download and extract both `twitter-media-collector-0.2.2.zip` and `twitter-media-archive-companion-0.1.0.zip` from [Releases](../../releases).
...
4. Restart Obsidian and enable `Twitter Media Archive Companion` under Community plugins.
...
After an update, reload the extension from `chrome://extensions`, reload open Twitter (now X) tabs, and restart Obsidian.

## Usage

1. Save one post with the save button added to posts on Twitter (now X). For bulk saving, open the target profile's `/media` page and choose **一括DL** → **開始**.
2. Open the viewer from the image icon in Obsidian's left ribbon or run `Open archive gallery`.
3. Import pending jobs with the `Import pending Twitter media jobs` download icon in the viewer header.
...
1. Twitter Media Collector saves media under `~/Downloads/XMediaClone/_jobs/<jobId>/media/` and publishes post metadata under `_manifest/<attemptUuid>/`. `XMediaClone` is a legacy internal name retained for compatibility.
...
├─ docs/ARCHIVE_JOB_V1.md          Collector → Companion handoff contract
...
Chrome IndexedDB is the operational deduplication ledger. Job manifests contain no cookies or tokens. In post notes, content after `<!--xmc:user-->` belongs to the user and is preserved during re-import. The names `x-media-*`, `XMediaClone`, `XMediaArchive`, and `xmc` remain as compatibility identifiers for existing installations. See [`docs/ARCHIVE_JOB_V1.md`](docs/ARCHIVE_JOB_V1.md) for the complete job contract.

## Troubleshooting and known limitations

- If the save button is missing, reload the extension from `chrome://extensions` and then reload the Twitter (now X) tab.
...
Twitter Media Collector uses the `downloads`, `storage`, and `unlimitedStorage` permissions plus host access for Twitter (now X). The Companion does not contact Twitter (now X), FxTwitter, oEmbed, or similar services, and it does not read Twitter cookies or tokens. Credentials are never stored in jobs.
...
[MIT License](LICENSE). This project is not affiliated with X Corp. (the operator of Twitter) or Obsidian.
